<template>
  ...
  <div>
    <div>
      ...
      <button class="btn btn-primary" type="submit" @click="editBook(this.$route.params.id)">Editar</button>
    </div>
  </div>
</template>
...
export default {
  ...
  methods: {
    getBook(id){
      ...
      axios.get(`http://localhost:3000/books/${id}`, headers)
      .then((res) => {
    ...
    editBook(id){
      ...
      axios.put(`http://localhost:3000/books/${id}`, params, headers)
      .then((res) => {
